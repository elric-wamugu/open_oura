...
enum EventStore {
    ...
    // real sync order (captured_unix, then ds) and splitting on any large backward jump
    ...
    static func epochs(_ events: [Ev]) -> [Epoch] {
        let order = events.map { (cu: $0.cu, ds: $0.ds) }
            .sorted { $0.cu != $1.cu ? $0.cu < $1.cu : $0.ds < $1.ds }
        var eps: [Epoch] = []
        for o in order {
